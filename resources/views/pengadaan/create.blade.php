        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.getElementById('generateBtn').addEventListener('click', function() {
                ...
                items.forEach((it, idx) => {
                    ...
                    bi.type = 'hidden';
                    bi.name = `items[${idx}][barang_id]`;
                    bi.value = it.barang_id;
                    const bq = document.createElement('input');
                    bq.type = 'hidden';
                    bq.name = `items[${idx}][qty]`;
                    bq.value = it.qty;

                    const bh = document.createElement('input');
                    bh.type = 'hidden';
                    bh.name = `items[${idx}][harga]`;
                    bh.value = it.harga; // default 0 kalau tidak ada
                    const bn = document.createElement('input');
                    bn.type = 'hidden';
                    bn.name = `items[${idx}][note]`;
                    bn.value = it.note ?? '';
                    form.appendChild(bi);
                    form.appendChild(bq);
                    form.appendChild(bh);
